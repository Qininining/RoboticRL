...
class FR5_Env(gym.Env):
    ...
    def init_env(self):
        ...
        '''
        # 构建 URDF 文件的绝对路径
        # 获取当前 Fr5_env.py 脚本文件所在的目录
        current_file_dir = os.path.dirname(os.path.abspath(__file__))

        # 假设 URDF 文件位于项目根目录下的 'fr5_description/urdf' 文件夹中
        # 'FR_Gym' 和 'fr5_description' 是同一个父目录下的兄弟文件夹
        urdf_path = os.path.join(
            current_file_dir, # 当前脚本的目录 (e.g., 'd:\Project\Git\Git\FR5_Reinforcement-learning-master\FR_Gym')
            "..",             # 返回上一级目录 (e.g., 'd:\Project\Git\Git\FR5_Reinforcement-learning-master')
            "fr5_description",
            "urdf",
            "fr5v6.urdf"
        )
        ...
        if not os.path.exists(urdf_path):
            raise FileNotFoundError(f"错误：URDF 文件未找到，请检查路径: {urdf_path}")

        # 创建机械臂，使用绝对路径
        self.fr5 = self.p.loadURDF(urdf_path,
                                 useFixedBase=True,
                                 basePosition=[0, 0, 0],
